...
$usland_team_related       = Usland::$options['team_related'];
...
<div class="container mt-5">
	<div class="row">
		<div class="con-title-column col-lg-12 col-md-12 col-sm-12">
			<!--Sec Title-->
			<div class="sec-title">
				<?php echo empty( $usland_team_related ) ? '' : '<h5>'. esc_html( $usland_team_related ).'</h5>';?>
			</div>
		</div>
		<div class="three-item-carousel col-lg-12 col-md-12 col-sm-12 padding-none other-property">
			<?php
				$managerid = get_the_ID();
				$the_query = new WP_Query(Usland_Helper::get_ulpropertym($managerid));
				if ( $the_query->have_posts() ) {
					while ( $the_query->have_posts() ) {
						$the_query->the_post();						
						global $post;
						$usland_pro_type    		                = get_post_meta( $post->ID, 'usland_pro_type', true );
						$manager    		                		= get_post_meta( $post->ID, 'usland_pro_manager_name', true );
			...
			<?php endif; } } else { }	wp_reset_postdata(); ?>
		</div>
	</div>
</div>
