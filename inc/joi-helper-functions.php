	...
	class Usland_Helper {
				...
				<div class="comment-inner">
					...
					<div class="comm-content">
						<h5><?php echo get_comment_author_link( $comment );?>
							<span><?php printf( __( '%1$s @ %2$s', 'usland' ), get_comment_date( '', $comment ), get_comment_time() );?></span>
						</h5>
						<?php if ( '0' == $comment->comment_approved ) : ?>
						<p><?php esc_html_e( 'Your comment is awaiting moderation.', 'usland' ); ?></p>
						<?php endif; ?>
						<?php comment_text(); ?>
					</div>
				</div>
		...
		public static function get_team(){
			$args = array(
		 	...
		 	$team[0] = __( 'Select a Team Member', 'usland' );
		...
		public static function get_ulproperty(){
			$args = array(
				'post_type' => 'ulproperty', 
				'orderby' => 'rand',
				'posts_per_page' => 3,
			);
			return $args;
		...
		public static function get_ulpropertym(){
			$args = array(
				'post_type' => 'ulproperty', 
				'orderby' => 'rand',
				'posts_per_page' => -1,
		...
		public static function get_ulpropertsingle(){
			$args = array(
				'post_type' => 'ulproperty', 
				'orderby' => 'rand',
				'posts_per_page' => -1,
			...
			$posts = get_posts( $args );
		 	$proper[0] = __( 'Select a Property', 'usland' );
		 	foreach ( $posts as $post ) {
		 		$proper[$post->ID] = $post->post_title;
		 	}
		 	return $proper;
		}
		...
		public static function get_main_menu(){
			...
			$nav_menus = array('default' => __('Default', 'usland')) + $nav_menus;
			return $nav_menus;
		}
